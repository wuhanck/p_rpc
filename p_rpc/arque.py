...
_logger = logging.getLogger('arque')


class Arque:
    stale_timeout = int(timedelta(days=1).total_seconds())
    ...
    @staticmethod
    def __get_keys(prefix):
        return {k: '{}:{}'.format(prefix, v) for k, v in Arque.s_keys.items()}

    @staticmethod
    def __get_stats_key(keys, task_id):
        return '{}:{}'.format(keys['stats'], task_id)

    def __init__(self, redis, prefix, requeue_limit=None, working_limit=None, stats_ttl=None):
        self._redis = redis
        self._keys = Arque.__get_keys(prefix)
        self._working_limit = working_limit or Arque.working_limit
        self._requeue_limit = requeue_limit or Arque.requeue_limit
        self._stats_ttl = stats_ttl or Arque.stats_ttl

    def _get_stats_key(self, task_id):
        return Arque.__get_stats_key(self._keys, task_id)
    ...
    @staticmethod
    def gen_task_id(task_timeout=None):
        task_timeout = task_timeout or Arque.stale_timeout
        task_id = '{}-{}'.format(uuid.uuid1().hex, task_timeout)
        return task_id

    @staticmethod
    async def enqueue(redis, prefix, task_data: bytes, task_id, delay=None, stats_ttl=None):
        """ Enqueue arbitrary bytes into working queue. enqueue/dequeue always in different place."""
        now = int(time.time())
        stats_ttl = stats_ttl or Arque.stats_ttl
        keys = Arque.__get_keys(prefix)
        stats_key = Arque.__get_stats_key(keys, task_id)
        with await redis as r:
            pipe = r.multi_exec()
            pipe.hset(keys['tasks'], task_id, task_data)
            pipe.hset(stats_key, 'enqueue_time', now)
            pipe.expire(stats_key, stats_ttl)
            if delay is None:
                pipe.lpush(keys['pending'], task_id)
            else:
                pipe.zadd(keys['delayed'], now + delay, task_id)
            await pipe.execute()

        _logger.debug(f'En-queued task ID: {task_id}. Data: {task_data}')
    ...
    async def dequeue(self, timeout=None):
        ...
        with await self._redis as r:
            task_id = await r.brpoplpush(self._keys['pending'], self._keys['pending'], timeout=timeout or 0)
        ...
        keys = [self._keys[k] for k in ('pending', 'working', 'tasks', 'failed', 'stats')]
        ...
        task_data = None if task_data == b'null' else task_data
        _logger.debug(f'De-queued task ID: {task_id}. Data: {task_data}')

        return task_id, task_data

    @staticmethod
    async def release(redis, prefix, task_id):
        """ Mark task as successfully processed. """
        keys = Arque.__get_keys(prefix)
        stats_key = Arque.__get_stats_key(keys, task_id)
        with await redis as r:
            pipe = r.multi_exec()
            pipe.zrem(keys['working'], task_id)
            pipe.hdel(keys['tasks'], task_id)
            pipe.delete(stats_key)
            await pipe.execute()

        _logger.debug(f'Released task ID: {task_id}.')
    ...
    async def sweep(self):
        ...
        keys = [self._keys[k] for k in ('pending', 'working', 'delayed', 'stats')]
        args = [int(time.time())]
        result = await self._call_script(script=script, keys=keys, args=args)
        _logger.info(f'Swept {result} tasks.')
    ...
    async def get_stats(self):
        ...
        with await self._redis as r:
            pipe = r.pipeline()
            pipe.hlen(keys['tasks'])
            pipe.llen(keys['pending'])
            pipe.zcard(keys['working'])
            pipe.zcard(keys['delayed'])
            pipe.llen(keys['failed'])
    ...
    async def get_task_stats(self, task_id):
        ...
        with await self._redis as r:
            result = await r.hgetall(stats_key)
        if not result:
            return None
        return {
